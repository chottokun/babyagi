...
def execute_task(task, task_list, OBJECTIVE):
    ...
    session_task_results.append({'id': task['id'], 'task': task['task'], 'content': task_output})
    pass
...
print("END")
#
print("session_task_results")
print(session_task_results)
sesseion_resuls = sorted(session_task_results, key=lambda x:x["id"])
print(sesseion_resuls)
line = ""
for s in sesseion_resuls:
    line += f"Task {s.get('id')} - {s.get['task']}:\n{s.get('content')}\n\n"

file = open(f'./output/output_sessions_{datetime.now().strftime("%d_%m_%Y_%H_%M_%S")}.txt', 'w')
file.write(line)
file.close()
